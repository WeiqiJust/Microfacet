...
void binder_woven2::set_param(const binder_woven_param &p)
{
	param = p;
}

void* binder_woven2::get_param()
{
	return &param;
}

void binder_woven2::generate_geom(const float density, const int num_area_hits_scalar)
{
	//processing geom
	r_geometry* g;
	...
	for (int i = 0; i < param.bisect_verts; i++)
	{
		...
		bisect[i].x = 0;
		bisect[i].y = param.bisect_w_y*cosf(t);
		bisect[i].z = param.bisect_h_y*sinf(t);
	}

	for (int i = -1; i <= 3; i++)
		pts[i+1] = Vector3((float)i/2/param.x_num, 0, (i % 2 != 0) ? param.bisect_h_y : param.bisect_h_y+param.height_y);
	...
	for (int i = 0; i < path_verts_y-1; i++)
	{
		...
		for (int j = 0; j < param.bisect_verts; j++)
		{
			y_woven.faces.push_back(triangle_face(idx + j, idx + ((j + 1) % param.bisect_verts), idx + param.bisect_verts + j));
			y_woven.faces.push_back(triangle_face(idx + param.bisect_verts + ((j + 1) % param.bisect_verts), idx + param.bisect_verts + j, idx + ((j + 1) % param.bisect_verts)));
		}
	}
...
void binder_woven2::generate(std::vector<instance_property*> &results,
							 ...
							 const std::vector<const std::vector<instance_property*>*> &neighbors)
{
	...
	for (int x = 0; x < param.x_num*2; x++)
		for (int y = 0; y < param.y_num; y++)
		{
			...
			p->setup_matrix(v_min+Vector3((float)(x)/param.x_num/2, (float)(y+((x % 2 == 1) ? 0.5 : 0))/param.y_num, 0), Vector3(1, 0, 0), Vector3(0, 1, 0), Vector3(0, 0, 1), 1);
			results.push_back(p);
		}
}
...
//
//	int path_verts_y = max(param.total_path_verts / param.y_num, 3);
//	base_tri_mesh y_woven(param.bisect_verts*path_verts_y, 0);
//	for (int i = 0; i < path_verts_y; i++)
//	{
//		float	global_t = (float)i/(path_verts_y-1)*2;
//		int		i_t = min((int)global_t, 1);
...
//		int idx = i*param.bisect_verts;
//		for (int j = 0; j < param.bisect_verts; j++)
//			y_woven.position(idx+j) = bisect[j]+Vector3(p.x, p.y, p.z);
//	}
//
//	for (int i = 0; i < path_verts_y-1; i++)
//	{
...
//		{
//			f.i2 = idx+j;
//			f.i1 = idx+((j+1) % param.bisect_verts);
//			f.i0 = idx+param.bisect_verts+j;
//			y_woven.add_face(f);
//			f.i2 = idx+param.bisect_verts+((j+1) % param.bisect_verts);
//			f.i1 = idx+param.bisect_verts+j;
//			f.i0 = idx+((j+1) % param.bisect_verts);
//			y_woven.add_face(f);
//		}
//	}
//	y_woven.compute_normal();
//
//	for (int i = 0; i < param.bisect_verts; i++)
//	{
//		int idx = (path_verts_y-1)*param.bisect_verts;
//		Vector3 n = y_woven.normal(i)+y_woven.normal(idx+i);
//		Normalize(n);;
//
//		y_woven.normal(i) = y_woven.normal(idx+i) = n;
//	}
//
//	g = new r_geometry(mff_singleton::get()->get_handle());
//	g->load(&y_woven);
//	mff_singleton::get()->assign_geom("bd_woven_y", smart_pointer<r_geometry*>(g));
//
//	//x_woven
//	for (int i = 0; i < param.bisect_verts; i++)
//	{
...
//
//	for (int i = -1; i <= 3; i++)
//		pts[i+1] = Vector3((float)i/2/param.x_num, 0, 
//				param.bisect_h_x+param.height_x/2);
//				//(i % 2 != 0) ? param.bisect_h_x : param.bisect_h_x+param.height);
//
//	int path_verts_x = max(param.total_path_verts / param.x_num, 3);
//	base_tri_mesh x_woven(param.bisect_verts*path_verts_x, 0);
//	for (int i = 0; i < path_verts_x; i++)
//	{
//		float	global_t = (float)i/(path_verts_x-1)*2;
//		int		i_t = min((int)global_t, 1);
...
//		int idx = i*param.bisect_verts;
//		for (int j = 0; j < param.bisect_verts; j++)
//			x_woven.position(idx+j) = bisect[j]+Vector3(p.x, p.y, p.z);
//	}
//
//	for (int i = 0; i < path_verts_x-1; i++)
//	{
...
//			f.i0 = idx+j;
//			f.i1 = idx+((j+1) % param.bisect_verts);
//			f.i2 = idx+param.bisect_verts+j;
//			x_woven.add_face(f);
//			f.i0 = idx+param.bisect_verts+((j+1) % param.bisect_verts);
//			f.i1 = idx+param.bisect_verts+j;
//			f.i2 = idx+((j+1) % param.bisect_verts);
//			x_woven.add_face(f);
//		}
//	}
//	x_woven.compute_normal();
//
//	for (int i = 0; i < param.bisect_verts; i++)
//	{
//		int idx = (path_verts_x-1)*param.bisect_verts;
//		Vector3 n = x_woven.normal(i)+x_woven.normal(idx+i);
//		Normalize(n);;
//
//		x_woven.normal(i) = x_woven.normal(idx+i) = n;
//	}
//
//	g = new r_geometry(mff_singleton::get()->get_handle());
//	g->load(&x_woven);
//	mff_singleton::get()->assign_geom("bd_woven_x", smart_pointer<r_geometry*>(g));
//
//	//prcessing geom_hierarchy
//	char name[2][32] = {"bd_woven_y", "bd_woven_x"};
//
...
//		smart_pointer<r_geom_hierarchy*> sp_gh = mff_singleton::get()->get_geom_hierarchy(name[i]);
//
//		if (sp_gh != NULL)
//		{
...
//			scales.push_back(pp);
//			p_gh->set_scales(scales);
//			mff_singleton::get()->assign_geom_hierarchy(name[i], smart_pointer<r_geom_hierarchy*>(p_gh));
//		}
//		p_gh->sample_points(density, num_area_hits_scalar);
//	}
//}
