...
vector<string> split_filename(const string name)
{
...
void render_grid_plane_prediction(MicrofacetEditor& m_editor)
{
	...
	string samplefile = "T:/Microfacet/data/cube_texture/cubelight.txt";
	m_editor.load_sky_box(0, samplefile);
	string grayfile = "T:/Microfacet/output/result/predict_gray.txt";
	string colorfile = "T:/Microfacet/output/result/predict_color.txt";
	string path = "T:/Microfacet/output/result/";
	...
	for (int i = 0; i < count_g; i++)
	{
		grid_plane param;
		string filename;
		fpg >> filename >> param.roughness >> param.scale >> param.x >> param.y;
