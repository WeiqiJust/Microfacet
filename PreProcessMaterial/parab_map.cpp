...
void parab_frame::compute_spherical_area(const int num_samples)
{
	...
	for (int i = 0; i < num_samples; i++)
	{
		...
		//	printf_s("Time = %gsecs\n", tt.elapsed_time());
		if (i%100000000 == 0)
			cout << "single compute "<<printsize++ << endl;
	}
...
void double_parab_frame::compute_spherical_area(const int num_samples)
{
	...
	Vector3 v;
	float pdf;
	//hammersley seq(256, num_samples*2);
	//float* result = seq.get_sample();
	for (int i = 0; i < num_samples; i++)
	{

		//float* result = seq.get_sample();
		//uniform_sphere_sample(v, result[2*i], result[2*i+1]);
		...
		if (ii >= 0)
			count[idx[ii]]++;
		if (i % 100000000 == 0)
			cout << "double compute "<< printsize++ << endl;
	}
	//cout << "double_parab_frame count size = " << count.size() << endl;
